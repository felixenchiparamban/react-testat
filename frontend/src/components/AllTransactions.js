   ...
   constructor(){
    ...
    getAllTransaction(){
        ...
        api.getTransactions(this.props.token, fromDate, toDate, 10, this.state.skip)
            .then(data => {console.log(data); this.setState({transactions : data.result}) });
    };
                  ...
                  <div>
                    ...
                    <div>
                      ...
                      <select onChange={this.onMonthChange}>
                          { months.map((month, index) => { return <option key={month} value={index}>{month}</option>}) }
                      </select>
                  </div>
                  <div>
                    <table className="ui definition table">
                      ...
                      <Button.Group>
                          <Button labelPosition='left' icon='left chevron' content='Back' onClick={this.previourPage} disabled={this.state.skip === 0} />
                          <Button labelPosition='right' icon='right chevron' content='Forward' onClick={this.nextPage} disabled={this.state.skip === 90}/>
                      </Button.Group>
                  </div>
                  </div>
                </div>
